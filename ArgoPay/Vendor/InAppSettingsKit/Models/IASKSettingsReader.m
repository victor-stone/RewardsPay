...
- (id) initWithSettingsFileNamed:(NSString*) fileName
    ...
    if (self) {
        ...
        self.localizationTable = _settingsDictionary[@"StringsTable"];
...
- (void)_reinterpretBundle:(NSDictionary*)settingsBundle {
    ...
    for (NSDictionary *specifier in preferenceSpecifiers) {
        if ([self.hiddenKeys containsObject:specifier[kIASKKey]]) {
            continue;
        }
...
- (NSString *)locateSettingsFile: (NSString *)file {
    ...
    NSArray *languageFolders = @[[[NSLocale preferredLanguages][0] stringByAppendingString:kIASKBundleLocaleFolderExtension],
                                 @""];
